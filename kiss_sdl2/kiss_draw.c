...
SDL_Renderer* kiss_init(char* title, kiss_array *a, int w, int h)
{
	...
	r += kiss_font_new(&kiss_textfont, font, a,
		kiss_textfont_size);
	r += kiss_font_new(&kiss_buttonfont, font, a,
		kiss_buttonfont_size);
	r += kiss_image_new(&kiss_normal, normal, a, renderer);
	r += kiss_image_new(&kiss_prelight, prelight, a, renderer);
	r += kiss_image_new(&kiss_active, active, a, renderer);
	r += kiss_image_new(&kiss_bar, bar, a, renderer);
	r += kiss_image_new(&kiss_vslider,vslider, a, renderer);
	r += kiss_image_new(&kiss_hslider, hslider, a, renderer);
	r += kiss_image_new(&kiss_up, up, a, renderer);
	r += kiss_image_new(&kiss_down, down, a, renderer);
	r += kiss_image_new(&kiss_left, left, a, renderer);
	r += kiss_image_new(&kiss_right, right, a, renderer);
	r += kiss_image_new(&kiss_combo, combo, a, renderer);
	r += kiss_image_new(&kiss_selected, selected, a, renderer);
	r += kiss_image_new(&kiss_unselected, unselected, a,
		renderer);
	if (r) {
		kiss_clean(a);
		return NULL;
